...
// Ensure data is sorted by Mun_ID and Year
sort MUN_ID year

// Generate a lead variable for Revenue, leading by 3 years
by MUN_ID: gen lead_Rev_3years = F3.New_Rev_PC

// Create the Placebo variable with the value of Revenue from three years in the future
gen Placebo = lead_Rev_3years


// Again, on positive observations, as discussed in paper
drop if missing(Placebo)

// Drop temporary variables if not needed
drop lead_Rev_3years
...
// ============== 		GPS Estimation			==============
// ===========================================================
gen Placebo_int = int(Placebo)

centile Placebo_int, centile(25 50 75)
return list

gen cut=0
replace cut = r(c_1) if Placebo_int<=r(c_1)
replace cut = r(c_2) if Placebo_int>r(c_1) & Placebo_int<=r(c_2)
replace cut = r(c_3) if Placebo_int>r(c_2)


gpscore2 laggedAVGWindDensity lagged_Turbines laggedUnemplyRate, t(Placebo_int) gpscore(gpscore) link(log) family(gamma) predict(gpscore_predicted) sigma(ML) cutpoints(cut) index(mean) nq_gps(2) detail

* Most Relevant
eststo clear
local outcomes_Relevant_Delta "delta_income_Personal  delta_Budget_T delta_Emp_Tot_PC"
foreach VVV in `outcomes_Relevant_Delta' {
qui: eststo: xtreg `VVV' Placebo i.year gpscore, vce(cluster MUN_ID)
qui: estadd local YFE "Yes"
}
estimates table est1 est2 est3, star(0.1 0.05 0.01) stats(N, N_g, g_min, g_max, j, df_m, chi2, hansenp, han_df)

esttab using "Timing_Placebo_Tables\Relevant_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
posthead("\multicolumn{4}{l}{\parbox{11cm}{Outcome variable: Change in indicated outcome, normalised per capita. }} \\ \multicolumn{4}{l}{\parbox{11cm}{Explanatory variables: Placebo Treatment: Revenues Three Years in the Future, GPS}} \\ \midrule & Total Income & Total Budget & Total Employment \\ \toprule") ///
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo Tests - Varied Timing \label{relevantplacebo}) ///
...
esttab using "Timing_Placebo_Tables\Incomes_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
posthead("\multicolumn{7}{l}{\parbox{11cm}{Outcome variable: Change in income by type, normalised per capita. }} \\ \multicolumn{7}{l}{\parbox{11cm}{Explanatory variables: Placebo Treatment: Revenues Three Years in the Future, GPS}} \\ \midrule & Total & Entre. & Wages & Pensions & Educ. & Benefits \\ \toprule") ///
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo of Wind Power Deployment on the Personal Income - Varied Timing \label{incomeplacebo}) ///
nomtitle nonumbers nonotes nogaps ///
note(p-values in parentheses, $^{*}\, p<0.1$, $^{**}\, p<0.05$, $^{***}\, p<0.01$) width(\hsize) replace
...
esttab using "Timing_Placebo_Tables\Budgets_Total_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
posthead("\multicolumn{7}{l}{\parbox{11cm}{Outcome variable: Change in budget by type, normalised per capita. }} \\ \multicolumn{7}{l}{\parbox{11cm}{Explanatory variables: Placebo Treatment: Revenues Three Years in the Future, GPS}} \\ \midrule & Total & Admin & Educ. & Health & Housing & Utilities & Infra \\ \toprule") ///
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo of Wind Power Deployment on the Municipal Budget - Varied Timing \label{budgetplacebo}) ///
nomtitle nonumbers nonotes nogaps ///
note(p-values in parentheses, $^{*}\, p<0.1$, $^{**}\, p<0.05$, $^{***}\, p<0.01$) width(\hsize) replace
...
esttab using "Timing_Placebo_Tables\Employment_detailed1_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
...
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo of Wind Power Deployment on Employment - Varied Timing \label{employmentplacebo1}) ///
nomtitle nonumbers nonotes nogaps ///
note(p-values in parentheses, $^{*}\, p<0.1$, $^{**}\, p<0.05$, $^{***}\, p<0.01$) width(\hsize) replace
...
esttab using "Timing_Placebo_Tables\Employment_detailed2_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
...
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo of Wind Power Deployment on Employment - Varied Timing \label{employmentplacebo2}) ///
nomtitle nonumbers nonotes nogaps ///
note(p-values in parentheses, $^{*}\, p<0.1$, $^{**}\, p<0.05$, $^{***}\, p<0.01$) width(\hsize) replace
...
esttab using "Timing_Placebo_Tables\Employment_detailed3_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
...
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo of Wind Power Deployment on Employment - Varied Timing \label{employmentplacebo3}) ///
nomtitle nonumbers nonotes nogaps ///
note(p-values in parentheses, $^{*}\, p<0.1$, $^{**}\, p<0.05$, $^{***}\, p<0.01$) width(\hsize) replace
...
esttab using "Timing_Placebo_Tables\Employment_detailed4_Placebos.tex", keep(Placebo gpscore) booktabs ///
varlabels(Placebo "\addlinespace Placebo" gpscore "GPS", elist(Placebo "\addlinespace" gpscore "\addlinespace" )) ///
...
stat(YFE N, labels("\addlinespace Year FE" "\addlinespace \midrule \addlinespace Observations" "Adj. R-squared")) ///
p star(* 0.1 ** 0.05 *** 0.01) ///
title(Placebo of Wind Power Deployment on Employment - Varied Timing \label{employmentplacebo4}) ///
nomtitle nonumbers nonotes nogaps ///
note(p-values in parentheses, $^{*}\, p<0.1$, $^{**}\, p<0.05$, $^{***}\, p<0.01$) width(\hsize) replace
eststo clear