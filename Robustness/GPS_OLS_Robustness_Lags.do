...
qui: eststo: xtreg `VVV' lagged_Rev_PC lagged_2_Rev_PC i.year gpscore laggedAVGWindDensity, vce(robust)
...
local outcomes_income_Delta "delta_income_Personal  delta_income_Entre delta_income_Wages delta_income_Pensions delta_income_Edu delta_income_Daily"
foreach VVV in `outcomes_income_Delta' {
qui: eststo: xtreg `VVV' lagged_2_Rev_PC lagged_3_Rev_PC i.year gpscore laggedAVGWindDensity, vce(robust)
qui:estadd local MFE "Yes"
qui: estadd local YFE "Yes"
}
estimates table est1 est2 est3 est4 est5 est6, star(0.1 0.05 0.01) stats(N, N_g, g_min, g_max, j, df_m, chi2, hansenp, han_df)
...
* Lags 1-2
eststo clear
local outcomes_pensions_detail "Inc_PnsnsOther_PC Inc_Pnsns_ATP_PC Inc_PnsnsSoc_PC Inc_Pnsn_Supplmtn_PC Inc_EarlyRtrmnt_PC Inc_EarlyRtrmntPay_PC"
foreach VVV in `outcomes_pensions_detail' {
qui: eststo: xtreg `VVV' lagged_Rev_PC lagged_2_Rev_PC i.year gpscore laggedAVGWindDensity, vce(robust)
qui: estadd local MFE "Yes" 
qui: estadd local YFE "Yes"
}
